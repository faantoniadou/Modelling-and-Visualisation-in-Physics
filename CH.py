...
def update(grid):     #frameNum, img, grid):
    
    mu = -a * grid + a * grid ** 3 - (k / dx**2) * ( np.roll(grid, 1, axis=0) + np.roll(grid, -1, axis=0) + 
...
def free_energy_density(grid):
    return np.sum(-(a/2) * grid ** 2 + (a/4) * grid ** 4 + k / (2 * dx ** 2) \
        * (np.roll(grid, 1, axis=0) - grid) ** 2 + (np.roll(grid, 1, axis=1) - grid) ** 2 )
...
def simulation(animate=False):
    ...
    plt.imshow(grid, vmax=1, vmin=-1, animated=True, cmap='plasma') 
    plt.colorbar()
    ...
    np.savetxt('free_energies.csv', free_energies, delimiter=',')
    

def plot_energy():
    x = np.arange(0,int(1e+6)+100,100)
    energies = np.genfromtxt("free_energies.csv")
